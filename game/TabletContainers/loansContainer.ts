...
export class loansContainer extends Phaser.GameObjects.Container {
  scene: Phaser.Scene;
  ...
  constructor(
    ...
    this.eventCenter.turnEventOn(
      ...
      () => {
        ...
        this.money.setText("$" + this.stateGlobal.loansModule.totalDebt)
        this.scrollPanel.setItems(this.stateGlobal.loansModule.possibleLoans);
    ...
    const moneyText = this.scene.add.text(-275, 0, "Dinero adeudado", {
      fontFamily: "MontserratSemiBold",
      fontSize: "18px",
      color: "#32F9C0",
      align: "center",
    }).setOrigin(0.5)
    this.money = this.scene.add.text(0, 0, "$" + this.stateGlobal.loansModule.totalDebt, {
    ...
    function createInversionHeaderCell(scene: Phaser.Scene) {
      ...
      var quote = rexUI.add.label({
        width: colWidth[1],
        text: scene.add.text(0, 0, "Cuota", {
          fontFamily: "MontserratSemiBold",
          fontSize: "18px",
          color: "#ffffff",
      ...
      return sizer
        ...
        .add(quote, 0, "top", { left: 10, right: 10, top: 10, bottom: 10 }, true)
        .add(
          price,
          0,
          "top",
    ...
    const acceptLoan = (item: InversionAndLoanType, sizer: rexUI.Sizer, button: Label) => {
      this.stateGlobal.loansModule.takeLoan(item);
    ...
    const cancelLoan = (item: InversionAndLoanType, sizer: rexUI.Sizer, button: Label) => {
      this.stateGlobal.loansModule.payLoan(item.id);
      this.scrollPanel.setItems(this.stateGlobal.loansModule.possibleLoans);
    ...
    function createInversionCell(
      ...
      const timeToPayment = "$" + Math.round(item.startAmount / item.percent).toString()

      var time = rexUI.add.label({
        width: colWidth[1],
        height: 40,
        text: scene.add.text(0, 0, timeToPayment, {
          color: "#FFFFFF",
          fontSize: "18px",
          align: "center",
          fixedWidth: colWidth[1],
      ...
      var button1 = rexUI.add
        .label({
          ...
          text: scene.add.text(0, 0, "Solicitar", {
            color: "#3340736",
            fontSize: "16px",
            align: "center",
            fixedWidth: 80,
    ...
    const checkIfInversionInside = (_inversion: InversionAndLoanType) => {
      return this.stateGlobal.loansModule.loans.find((inversion: InversionAndLoanType) => (inversion.id === _inversion.id))
    }
    ...
    this.scrollPanel = rexUI.add.gridTable({
      ...
      createCellContainerCallback: function (cell, cellContainer) {
        ...
        if (r) {
          if (item.startAmount - item.holdAmout > availableMoney) {
            button1.setAlpha(0);
          ...
          if (item.state === "disabled") {
            ...
            if (Number(item.id)*50 > stateGlobal.reputation) {
              button1.setAlpha(0.2);
              button2.setAlpha(0.2);
              button1.removeInteractive();
              button1.removeInteractive();
      ...
      items: this.stateGlobal.loansModule.possibleLoans,
