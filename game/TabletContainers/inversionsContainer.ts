...
export class inversionsContainer extends Phaser.GameObjects.Container {
  scene: Phaser.Scene;
  ...
  constructor(
    ...
    this.eventCenter.turnEventOn(
      ...
      () => {
        ...
        this.scrollPanel.setItems(this.stateGlobal.inversionModule.possibleInversions.filter((inversion: InversionAndLoanType) => inversion.inversionType === "fixed"));
    ...
    function createInversionHeaderCell(scene: Phaser.Scene) {
      ...
      var time = rexUI.add.label({
        width: colWidth[1],
        text: scene.add.text(0, 0, "Tiempo", {
          fontFamily: "MontserratSemiBold",
          fontSize: "18px",
          color: "#ffffff",
    ...
    function createInversionCell(
      ...
      const timeToPayment = Math.round(item.periodCycle / 8).toString() + " días"
    ...
    this.scrollPanel = rexUI.add.gridTable({
      ...
      items: (this.stateGlobal.inversionModule.possibleInversions as InversionAndLoanType[]).filter((inversion: InversionAndLoanType) => inversion.inversionType === "fixed"),
