...
class DialogueManager {
  ...
  createContainer(text: string, config?: DialogConfig) {
    ...
    this.container.add([graphics, this.textDisplayed])
  ...
  nextText() {
    ...
    if (
      ...
      this.scene.time.delayedCall(this.config[this.textCounter].keepAlive, () => {
        this.textCounter += 1;
        if(this.textDisplayed && this.textDisplayed.active) {
          this.textDisplayed?.setText("");
          this.stateFunctions.forEach((fn) => {
            fn("CONTINUE", this.texts[this.textCounter]);
          });
